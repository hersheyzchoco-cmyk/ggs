...
local BANNED_USERS = {"TsukuyomiNoMikoto_YT"}
...
local function buildMainUI(mode)
    ...
    -- Setup paper loose-leaf lines on yellow note
    for i = 0, 5 do
        new("Frame", {
    ...
    -- Setup top of Yellow Note items
    ...
    if mode == "Banned" then
        Greeting.Text = "BANNED! ❗️"
        Greeting.TextColor3 = Color3.fromRGB(210, 50, 50)
        Arrow.Visible = false
        SubGreeting.Text = "early christmas gift! 🎁💞"
        SubGreeting.TextColor3 = Color3.fromRGB(150, 60, 60)

        new("TextLabel", {
            Size = UDim2.new(1, -20, 0, 40), Position = UDim2.new(0, 10, 0, 84),
            BackgroundTransparency = 1,
            Text = "Account: " .. LocalPlayer.Name .. "\nID: " .. tostring(LocalPlayer.UserId),
            TextColor3 = Color3.fromRGB(120, 70, 70), TextSize = 13, Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 16, Parent = MainNote,
        })

        new("TextLabel", {
            Size = UDim2.new(1, -20, 0, 40), Position = UDim2.new(0, 10, 0, 215),
            BackgroundTransparency = 1,
            Text = "Access permanently revoked — appeals will not be accepted.",
            TextColor3 = Color3.fromRGB(160, 60, 60), TextSize = 12, Font = Enum.Font.GothamMedium,
            TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true, ZIndex = 16, Parent = MainNote,
        })

        GoLbl.Text = "💬  Join Discord"
        GoBtn.BackgroundColor3 = Color3.fromRGB(88, 101, 242)

        GoBtn.MouseEnter:Connect(function() tween(GoBtn, { BackgroundColor3 = Color3.fromRGB(110, 122, 255) }, 0.15) end)
        GoBtn.MouseLeave:Connect(function() tween(GoBtn, { BackgroundColor3 = Color3.fromRGB(88, 101, 242) }, 0.2) end)
        GoBtn.MouseButton1Click:Connect(function()
            pcall(function() if setclipboard then setclipboard(DISCORD_LINK) end end)
            GoLbl.Text = "✓  Copied Link!"
    ...
    elseif mode == "Lobby" then
        Greeting.Text = "LOBBY! ⚠️"
        Greeting.TextColor3 = Color3.fromRGB(220, 150, 20)
        Arrow.Visible = false
        SubGreeting.Text = "cannot execute in lobbies!"
        SubGreeting.TextColor3 = Color3.fromRGB(140, 100, 30)
        ...
        new("TextLabel", {
            Size = UDim2.new(1, -20, 0, 20), Position = UDim2.new(0, 10, 0, 215),
            BackgroundTransparency = 1,
            Text = "Current Place: " .. tostring(game.PlaceId) .. " (lobby)",
            TextColor3 = Color3.fromRGB(110, 95, 60), TextSize = 12, Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 16, Parent = MainNote,
        })

        GoLbl.Text = "✕  Close Interface"
        GoBtn.BackgroundColor3 = Color3.fromRGB(180, 70, 70)

        GoBtn.MouseEnter:Connect(function() tween(GoBtn, { BackgroundColor3 = Color3.fromRGB(210, 90, 90) }, 0.15) end)
        GoBtn.MouseLeave:Connect(function() tween(GoBtn, { BackgroundColor3 = Color3.fromRGB(180, 70, 70) }, 0.2) end)
        GoBtn.MouseButton1Click:Connect(function()
            closeUI()
        end)

    else -- NORMAL MODE ("Key")
        Greeting.Text = "heyyy " .. LocalPlayer.Name .. " 👋"
        SubGreeting.Text = "drop ur key to get in!"
        ...
        SmallBtnRow = new("Frame", { Size = UDim2.new(1, -20, 0, 34), Position = UDim2.new(0, 10, 0, 206), BackgroundTransparency = 1, ZIndex = 16, Parent = MainNote })
    ...
    -- validation UI responses (Only compiled in "Key" mode)
    if mode == "Key" then
...
-- SAFETY ROUTING & EXECUTION CHECKS
